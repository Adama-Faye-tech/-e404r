...
Despliega E404R en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.e404r` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/e404r
sudo chown $USER:$USER /var/lib/e404r
```
...
Crea `/etc/nginx/sites-available/e404r`:
...
sudo ln -s /etc/nginx/sites-available/e404r /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de E404R
...
tar -czf e404r-backup-$(date +%Y%m%d).tar.gz /var/lib/e404r

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/e404r-$(date +\%Y\%m\%d).tar.gz /var/lib/e404r
```
...
```bash
# Verificar si E404R está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/e404r
chmod 755 /var/lib/e404r
```
