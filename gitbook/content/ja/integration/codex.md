...
E404RをOpenAI Codex CLIと統合し、OpenAI APIリクエストをE404Rのインテリジェントルーティングシステム経由でルーティングします。

## 前提条件

- OpenAI Codex CLIがインストール済み
- E404Rがローカルで動作中、またはクラウドエンドポイントが設定済み
...
1. E404Rが動作中か確認: `curl http://localhost:20128/health`
...
1. モデル名がE404R設定と一致するか確認
2. E404RダッシュボードでOpenAIプロバイダー接続がアクティブか確認
3. 接続されたプロバイダーでモデルが利用可能か確認

## クラウドエンドポイント

localhostの代わりにE404Rクラウドエンドポイントを使用するには:

```bash
export OPENAI_BASE_URL="https://e404r.com"
```

E404RクラウドダッシュボードでAPIキーが設定されていることを確認してください。
