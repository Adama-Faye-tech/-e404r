...
Tích hợp E404R với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của E404R.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- E404R đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint E404R:
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng E404R để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua E404R
...
3. Nhận giải thích AI qua E404R

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng E404R để hiểu context và thực hiện thay đổi
...
1. Xác minh E404R đang chạy: `curl http://localhost:20128/health`
...
2. Xác minh E404R instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs E404R để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng E404R cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://e404r.com`
2. Đảm bảo bạn đã cấu hình API key trong E404R cloud dashboard
...
## Tích hợp với Tính năng E404R

### Định tuyến Model

E404R tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, E404R tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua E404R dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
