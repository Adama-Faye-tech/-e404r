...
E404R is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[E404R Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.e404r/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.e404r/usage.json`, `~/.e404r/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.e404r/db.json`)
- usage stats: `~/.e404r/usage.json`
- request log lines: `~/.e404r/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[E404R Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.e404r` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/e404r && npm run build`
- Build Docker image: `cd /root/dev/e404r && docker build -t e404r .`
