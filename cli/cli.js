...
// Subcommands (`e404r xai video …`) run against an already-running gateway
...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.e404r/runtime
...
// Identifiers for killAllAppProcesses - only kill e404r specifically
const PROCESS_IDENTIFIERS = [
  'e404r'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "e404r")
    : path.join(os.homedir(), ".e404r");
}
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running e404r/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "e404r" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("e404r") && (cmd.includes("cli.js") || cmd.includes("\\e404r") || cmd.includes("/e404r")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running e404r/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "e404r".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("e404r") && (cmd.includes("cli.js") || cmd.includes("/e404r")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 E404R is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 E404R is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "e404r", "db.json") : path.join(".e404r", "db.json"));
