...
async function buildCodexHeader() {
  ...
  const { installed, hasE404R, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasE404R, settings } = result.data;
  ...
  // Extract E404R custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:E404R-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract E404R provider config
  const provider = settings?.models?.providers?.["e404r"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("e404r/") ? primary.replace("e404r/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasE404R, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use E404R\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
