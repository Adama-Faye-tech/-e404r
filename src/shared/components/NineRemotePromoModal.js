...
const E404R_REMOTE_URL = "http://localhost:20127/dashboard/console-log";
      ...
      <div className="relative w-full max-w-sm rounded-[14px] overflow-hidden shadow-[var(--shadow-elev)] fade-in flex flex-col bg-surface border border-border-subtle">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-border-subtle">
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 rounded-[8px] flex items-center justify-center bg-[#00a8cc]">
              <span className="material-symbols-outlined text-white text-base">terminal</span>
            </div>
            <span className="text-xs font-bold uppercase tracking-wider text-[#00a8cc] font-mono">E404R Remote</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-7 py-7 pb-9 flex flex-col gap-6">
          ...
          <div className="flex flex-col items-center gap-2 text-center mt-2">
            <div className="w-14 h-14 rounded-[14px] flex items-center justify-center mb-1 bg-[#00a8cc] shadow-[var(--shadow-warm)]">
              <span className="material-symbols-outlined text-white text-[30px]">terminal</span>
            </div>
            <h1 className="text-lg font-bold text-text-main tracking-tight">E404R Remote</h1>
          ...
          <div className="flex gap-2 w-full">
            {FEATURES.map(({ icon, label, desc }) => (
              <div key={label} className="flex-1 flex flex-col items-center gap-1.5 py-4 px-1 rounded-[10px] border border-border-subtle bg-surface-2">
                <span className="material-symbols-outlined text-[#00a8cc] text-[22px]">{icon}</span>
          ...
          <div className="flex flex-col gap-3 w-full">
            {BULLETS.map(({ icon, text }) => (
              <div key={icon} className="flex items-center gap-2.5">
                <span className="material-symbols-outlined flex-shrink-0 text-[#00a8cc] text-[16px]">{icon}</span>
          ...
          {/* CTA */}
          <button
            onClick={() => window.open(E404R_REMOTE_URL, "_blank")}
            className="w-full py-3 flex items-center justify-center gap-2 text-sm font-semibold text-white rounded-[10px] hover:opacity-90 shadow-[var(--shadow-warm)] active:scale-[0.98] transition-all"
