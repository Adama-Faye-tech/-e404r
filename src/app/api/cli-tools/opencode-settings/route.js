...
const hasE404RConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["e404r"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["e404r"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasE404R: hasE404RConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("e404r/") ? config.model.replace(/^e404r\//, "") : null,
...
// POST - Apply E404R as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_e404r";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing e404r provider entry and its models
    const existingProvider = config.provider["e404r"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["e404r"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `e404r/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `e404r/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("e404r/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove E404R provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["e404r"]?.models) {
      delete config.provider["e404r"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["e404r"].models).length === 0) {
        delete config.provider["e404r"];
        if (config.model?.startsWith("e404r/")) delete config.model;
      } else if (config.model === `e404r/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["e404r"].models);
        config.model = `e404r/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire e404r provider
      if (config.provider) delete config.provider["e404r"];
      if (config.model?.startsWith("e404r/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("e404r/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "E404R settings removed from OpenCode",
    });
  } catch (error) {
