...
const hasE404RConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["e404r"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("e404r");
};
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "E404R", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "E404R settings removed from Kilo Code" });
  } catch (error) {
