...
// Check if settings has E404R config
const hasE404RConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["e404r"];
};

// Read per-agent models.json and return current model id (without "e404r/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["e404r"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasE404R: hasE404RConfig(settings),
...
// POST - Update E404R settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `e404r/${model}`;

    // Remove all old e404r/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("e404r/"))
    ...
    // Add fresh e404r models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`e404r/${m}`] = {};
    });

    // Remove old e404r model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("e404r/")) {
    ...
    // Update models.providers.e404r with all models
    settings.models.providers["e404r"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `e404r/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove E404R settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove E404R from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["e404r"];
    ...
    // Remove e404r models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("e404r/"));
    ...
    // Reset agents.defaults.model.primary if it uses e404r
    if (settings.agents?.defaults?.model?.primary?.startsWith("e404r/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "E404R settings removed successfully",
    });
  } catch (error) {
