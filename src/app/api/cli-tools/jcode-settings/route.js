...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-e404r.env");
};
...
const hasE404RConfig = (config) => {
  ...
  if (providers["e404r"]) return true;
...
export async function GET() {
  ...
  const hasE404R = hasE404RConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_E404R_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile e404r",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["e404r"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_E404R_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "e404r configuration removed from jcode",
    });
  } catch (error) {
