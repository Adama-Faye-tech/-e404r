...
const PROVIDER_NAME = "e404r";
const MODEL_SLOT = "e404r";
const BUILTIN_DEFAULT = "grok-build";

// [model.e404r] ... until next [section] header or EOF
...
const PREV_DEFAULT_RE = /^# e404r-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
...
const buildModelSection = (model, baseUrl, apiKey) => {
  const lines = [
    ...
    `base_url = "${baseUrl}"`,
    `name = "E404R"`,
    `description = "Routed via E404R gateway"`,
...
// Remember the previous default once (so re-Apply does not overwrite it with "e404r")
const rememberPrevDefault = (toml) => {
  ...
  const marker = `# e404r-prev-default = "${current}"\n`;
  // Prefer placing the marker just above [model.e404r] if present, else at EOF
...
export async function POST(request) {
  try {
    ...
    const keyToWrite = apiKey || "sk_e404r";
