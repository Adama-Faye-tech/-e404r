...
// Check if settings has E404R customModels
const hasE404RConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:E404R"));
};
...
// POST - Update E404R customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing E404R configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:E404R"));
...
// DELETE - Remove E404R customModels only (keep other settings)
