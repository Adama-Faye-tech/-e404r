...
const hasE404RConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "E404R");
};

const getE404REntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "E404R") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getE404REntry(config);
...
// POST - Apply E404R config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing E404R entry or append
    const idx = config.findIndex((e) => e.name === "E404R");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove E404R entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "E404R");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "E404R removed from Copilot config",
    });
  } catch (error) {
