...
// Collect PIDs of all e404r-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from e404r install dir or wrapper cli.js
        const isAppProcess = lower.includes("e404r") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("e404r") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "e404r");
  }
  return path.join(os.homedir(), ".e404r");
}
...
// Resolve npx/e404r binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx e404r` — works regardless of global bin path changes after npm i -g
